model{
  ...
  for(k in 1:max(site_no)){ #max(year_no)

  for(j in 1:max(season_weeks)){ #max(season_weeks) #Change to loop through sites as columns instead of season weeks, all years together
  ...
  #### Process Model
    ...
    # mu[1,j]~dnorm(lambda[1,j],tau_proc)
    # lambda[1,j] <- beta1  + beta2*mu.0 + beta3*covar1[1,j] + beta4*covar2[1,j] + beta5*covar2[1,j]^2
  ...
  for(j in 2:max(season_weeks)){
   ...
   # mu[1,j]~dnorm(lambda[1,j],tau_proc)
    ...
    #JB edits - now will loop through sites? define IC for each site
