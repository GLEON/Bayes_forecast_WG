...
ggplot(pre_gloeo3, aes(y = totalperL, x = dayofyr))+
  geom_point()+
  geom_line()+
  facet_wrap(site~year)

# LN gloeo
ggplot(pre_gloeo3, aes(y = ln_totalperL, x = dayofyr))+
  geom_point()+
  geom_line()+
  facet_wrap(site~year)
...
## Random Walk - Pre 2009 gloeo data for observation error 3 sites - no Fichter

```{r}
y = as.matrix(pre_gloeo4[,2:4]) # NO Fichter
...
GBR <- gelman.plot(jags.burn_RW_multi) #The point up to where the GBR drops below 1.05 is termed the "burn in" period
...
2 Dec 2022 - used 2007 & 2008 data from 3 sites (no Fichter), a/r=0.1, burnin = 30000
mean = 1.2813, SD = 1.663
...
1. Empirical mean and standard deviation for each variable,
   plus standard error of the mean:
           ...
           Mean     SD  Naive SE Time-series SE
tau_obs  1.2813 1.6630 0.0067890       0.047599
tau_proc 0.6337 0.2348 0.0009586       0.004201

2. Quantiles for each variable:

           2.5%    25%    50%   75% 97.5%
tau_obs  0.5061 0.7418 0.9421 1.269 4.269
tau_proc 0.2829 0.4634 0.5994 0.765 1.191
...
GBR <- gelman.plot(jags.burn_RW) #The point up to where the GBR drops below 1.05 is termed the "burn in" period
...
data <- list(y=y,
             ...
             a_proc = 0.001,
             r_proc = 0.001, 
             a_obs = 0.555, # informed observation error priors
             r_obs = 0.349,
             x_ic=-5,tau_ic=100)


 init <- list(list(tau_proc=0.001, tau_obs = 0.1,beta1=-0.5, beta2=-0.5),
                list(tau_proc=0.1,tau_obs = 1,beta1=0, beta2=0),
                list(tau_proc=1, tau_obs = 5,beta1=0.5,beta2=0.5))
...
model{
  ...
  #### Process Model 
  for(t in 2:WK){
    x[t]~dnorm(lambda[t],tau_proc)
    lambda[t] <- beta1 + beta2*x[t-1]
...
j.model_DLM   <- jags.model (file = textConnection(mod1),
                             data = data,
                             inits = init,
                             n.chains = 3)  
...
jags.out_DLM   <- coda.samples (model = j.model_DLM,
                            variable.names = c("beta1","beta2", "tau_proc","tau_obs"),
                            n.iter = 10000)
plot(jags.out_DLM)

summary(jags.out_DLM)

var.mat <- as.matrix(jags.out_DLM)
pairs(var.mat)
cor(var.mat)

# Calculate DIC
DIC.DLM <- dic.samples(j.model_DLM, n.iter=5000)
DIC.DLM
```
...
data <- list(y=y,
 ...
 init <- list(list(tau_proc=0.001, tau_obs = 0.1,tau_C1_proc = 0.01,tau_C2_proc = 0.01, beta1=-0.5, beta2=-0.5, beta3=-0.5, beta4=-0.5, beta5=-0.5),
...
model{
  ...
  for(t in 2:WK){
    x[t]~dnorm(lambda[t],tau_proc)
    lambda[t] <- beta1 + beta2*x[t-1] + beta3*covar1[t] + beta4*covar2[t] + beta5*covar2[t]^2 + yr[year_no[t]]
...
for(k in 1:totYr){
  yr[k] ~ dnorm(0, tau_yr) # k loop on yr, centered on 0
  }
...
## Dyanmic Linear with RY & Air Temp GDD
```{r}
...
## Dynamic linear with Random Year Effect - test just min water temp or GDD
- just GDD better than just min Water temp - GDD better than both together too based on DIC
- keep RY effects in there for both drivers
```{r}
...
data <- list(y=y,
 ...
 init <- list(list(tau_proc=0.001, tau_obs = 0.1,
                   tau_C2_proc = 0.01,
                   beta1=-0.5, beta2=-0.5, beta3=-0.5, beta4=-0.5, beta5=-0.5),
              list(tau_proc=0.1,tau_obs = 1,tau_C2_proc = 0.1, 
                   beta1=0, beta2=0, beta3=0, beta4=0, beta5=0),
              list(tau_proc=1, tau_obs = 5,tau_C2_proc = 1,
                   beta1=0.5,beta2=0.5,beta3=0.5, beta4=0.5, beta5=0.5))
...
jags.out_DL_RY   <- coda.samples (model = j.model_DL_RY,
                            variable.names = c("beta1","beta2","beta4","beta5", "tau_proc","tau_obs", "tau_yr", "yr"), #"beta4","beta5",
                            n.iter = 50000)
plot(jags.out_DL_RY)

summary(jags.out_DL_RY)
gelman.diag(jags.out_DL_RY)

GBR <- gelman.plot(jags.out_DL_RY) #The point up to where the GBR drops below 1.05 is termed the "burn in" period

burnin = 10000                              ## determine convergence
jags.burn <- window(jags.out_DL_RY,start=burnin)  ## remove burn-in
plot(jags.burn)
...
init <- list(list(tau_proc=0.001, tau_obs = 0.1,tau_C1_proc = 0.01,tau_C2_proc = 0.01, 
                  beta1=-2, beta2=-0.5, beta3=0, beta4=0, beta5=0),
...
mod1 = 
  "model {
      ...
      for(t in 1:WK){
          for(j in 1:site_no){  # Loop over all sites
  ...
  for(t in 2:WK){
    ...
    lambda[t,j] <- beta1  + beta2*mu[t-1,j] + beta3*covar1[t,j] + beta4*covar2[t,j] + beta5*covar2[t,j]^2+ yr[year_no[t]] + alpha_site[j]
    }
  }
...
for(j in 1:site_no){ 
 ...
 # end loop over sites
  ...
  #### Random Year Effect
  for(k in 1:totYr){
    yr[k] ~ dnorm(0, tau_yr) # k loop on yr, centered on beta1 instead of 0
  }
...
### Worse option to have site loop outside everything - low convergence for site effects
mod1 = 
  "model {
...
j.model_DL_RY_RS_3sites   <- jags.model (file = textConnection(mod1),
                             data = data,
                             inits = init,
                             n.chains = 3)  
...
jags.out_DL_RY_RS_3sites  <- coda.samples (model = j.model_DL_RY_RS_3sites,
                            variable.names = c("beta1","beta2","beta3","beta4","beta5", "tau_proc","tau_obs", "tau_yr", "yr", "alpha_site", "tau_site"), #"yr",
                            n.iter = 50000) #50000

 
plot(jags.out_DL_RY_RS_3sites)

summary(jags.out_DL_RY_RS_3sites)
gelman.diag(jags.out_DL_RY_RS_3sites)

GBR <- gelman.plot(jags.out_DL_RY_RS_3sites) #The point up to where the GBR drops below 1.05 is termed the "burn in" period

burnin = 10000                              ## determine convergence
jags.burn <- window(jags.out_DL_RY_RS_3sites,start=burnin)  ## remove burn-in
plot(jags.burn)
...
# Calculate DIC
DIC.multi_RY_RS <- dic.samples(j.model_DL_RY_RS_3sites, n.iter=5000)
DIC.multi_RY_RS
...
## Ho Model - single site (HC) 1 covariate - GDD
-Tested min water temp - bad, trouble converging, large DIC, switch to 1 covariate with GDD
...
model{
  ...
  for(t in 2:WK){
    ...
    lam[t]=beta1 + beta2*x[t-1] + beta4*covar2[t] + beta5*covar2[t]^2 #beta3*covar1[t]
...
jags.out_Hetro   <- coda.samples (model = j.model_Hetero,
                            variable.names = c("tau_obs","beta1","beta2","beta4","beta5", "alpha1", "alpha2"), #"tau_add",
                            n.iter = 50000)
plot(jags.out_Hetro)
...
j.model_Hetero2   <- jags.model (file = textConnection(mod1),
                             data = data,
                             inits = init,
                             n.chains = 3)  
...
jags.out_Hetro2   <- coda.samples (model = j.model_Hetero2,
                            variable.names = c("tau_obs","beta1","beta2","beta3","beta4","beta5", "alpha1", "alpha2"), #"tau_add",
                            n.iter = 50000)
plot(jags.out_Hetro2)

summary(jags.out_Hetro2)

gelman.diag(jags.out_Hetro2)

GBR <- gelman.plot(jags.out_Hetro2) #The point up to where the GBR drops below 1.05 is termed the "burn in" period


burnin = 30000                              ## determine convergence
jags.burn_Hetero2 <- window(jags.out_Hetro2,start=burnin)  ## remove burn-in
plot(jags.burn_Hetero2)

summary(jags.burn_Hetero2)
gelman.diag(jags.burn_Hetero2)

DL_var.mat <- as.matrix(jags.burn_Hetero2)
pairs(DL_var.mat)
cor(DL_var.mat)

# Calculate DIC
DIC.hetero <- dic.samples(j.model_Hetero2, n.iter=5000)
DIC.hetero 
  
```


# Random site effect with multiple driver homo model
```{r}
model{
