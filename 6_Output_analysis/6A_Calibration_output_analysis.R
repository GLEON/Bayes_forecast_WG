...
########################CALCULATE ASSESSMENT METRICS#####################################
  ...
  hoa[i,1] <- model_names[i]
    ...
    pred <- as.matrix(read_csv(file=file.path(paste("./5_Model_output/5.1_Calibration/",paste0(model_names[i],'_predicted_states.csv')))))
    ...
    #7. Pearson's r btwn predicted and observed in log space
    corr <- cor(mean_pred_log, obs = c(obs_log[1,],obs_log[2,],obs_log[3,],obs_log[4,],obs_log[5,],obs_log[6,]), method = "pearson", use = "complete.obs")
