...
#5. Calculate output analysis metrics on appended vardats
#6. Calculate uncertainty partitioning analysis metrics from relative varmat
#7. Repeat steps 1-6 for each forecast week
################################################################################
...
for (n in 1:length(forecast_weeks)){
  ...
  vardat.IC.P <- vardat <- as.matrix(read_csv(file=file.path(paste("./7_Model_ensemble/",paste0('ensemble.vardat.IC.P_',forecast_weeks[n],'.csv')))))
  vardat.IC.P.Pa <- vardat <- as.matrix(read_csv(file=file.path(paste("./7_Model_ensemble/",paste0('ensemble.vardat.IC.P.Pa_',forecast_weeks[n],'.csv')))))
  vardat.IC.P.Pa.D <- vardat <- as.matrix(read_csv(file=file.path(paste("./7_Model_ensemble/",paste0('ensemble.vardat.IC.P.Pa.D_',forecast_weeks[n],'.csv')))))
...
for (n in 1:length(forecast_weeks)){
  ...
  hoa <- matrix(NA,1,11)
    ...
    varMat <- as.matrix(read_csv(file=file.path(paste("./7_Model_ensemble/",paste0(model_name,'_varMat_',forecast_weeks[n],'.csv')))))
