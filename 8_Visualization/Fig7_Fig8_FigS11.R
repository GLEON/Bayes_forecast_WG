...
#setting up counters and vectors for for-loop
model_names <- c("RW_obs","AR","wtrtemp_min","wtrtemp_min_lag","wtrtemp_MA7","schmidt_med_diff","schmidt_max_lag","wnd_dir_2day_lag","precip","GDD","schmidt_and_temp","schmidt_and_precip","temp_and_precip","precip_and_GDD","ensemble")
model_labels <- c("RW","AR","MinWaterTemp","MinWaterTempLag","WaterTempMA","ΔSchmidt","SchmidtLag","WindDir","Precip","GDD","Schmidt+Temp","Schmidt+Precip","Temp+Precip","Precip+GDD","Ensemble")
forecast_weeks <- c(1:4)
plotdata <- NULL
...
plotdata <- as_tibble(plotdata) %>%
  mutate(V3 = as.double(V3),
         ...
         V6 = as.double(V6),
         V7 = as.double(V7),
         V2 = fct_relevel(V2, "RW","AR","MinWaterTemp","MinWaterTempLag","WaterTempMA","ΔSchmidt","SchmidtLag","WindDir","Precip","GDD","Schmidt+Temp","Schmidt+Precip","Temp+Precip","Precip+GDD","Ensemble"))
colnames(plotdata) <- c("model_name","mod_label","Initial Conditions","Parameter","Driver","Process","forecast_week")
plotdata$best_delPL <- c(NA,1,NA,NA,NA,1,NA,NA,1,NA,NA,NA,NA,NA,NA,rep(NA, times = 15),rep(NA, times = 15),NA,NA,NA,1,1,NA,NA,NA,NA,NA,1,NA,NA,NA,NA)
