# Script to download and wrangle Onset water temp data for Newbury from clean data file off EDI stored locally JAB
# Last updated 2020 April 10 - JB
# New updates 2022 Feb 3
...
dat4_subset1 <- dat4_subset[c(123, 161, 277, 320, 430, 464, 588, 616, 732, 782, 892, 935, 1044, 1201),]
...
dat7 <- full_join(dat6, dat4_subset4, by = c("date", "year", "NB.tempC_mean", "NB.tempC_median", "NB.tempC_min", "NB.tempC_max", "NB.tempC_sd")) %>%
...
dat4_lag1 <- left_join(sampling_dates_lag[,2], dat4_lag, by = "date_1weeklag") %>%
  rename(NB.tempC_mean_lag = NB.tempC_mean, NB.tempC_median_lag = NB.tempC_median, NB.tempC_min_lag = NB.tempC_min, NB.tempC_max_lag = NB.tempC_max,NB.tempC_sd_lag = NB.tempC_sd)
...
for (i in 1:nrow(dat4_fill3)) {
  wtr_mean_diff[i] <- dat4_fill3$NB.tempC_mean[i+7] - dat4_fill3$NB.tempC_mean[i]
  wtr_median_diff[i] <- dat4_fill3$NB.tempC_median[i+7] - dat4_fill3$NB.tempC_median[i]
  wtr_min_diff[i] <- dat4_fill3$NB.tempC_min[i+7] - dat4_fill3$NB.tempC_min[i]
  wtr_max_diff[i] <- dat4_fill3$NB.tempC_max[i+7] - dat4_fill3$NB.tempC_max[i]
  wtr_sd_diff[i] <- dat4_fill3$NB.tempC_sd[i+7] - dat4_fill3$NB.tempC_sd[i]
...
gdd1 <- dat4_fill3 %>% # use daily water temp data summary for May-Oct
  mutate(gdd = ((NB.tempC_max + NB.tempC_min)/2) - base_temp) %>%
...
write_csv(watertemp_all, "./00_Data_files/Covariate_analysis_data/onset_watertemp_all_NB.csv")
