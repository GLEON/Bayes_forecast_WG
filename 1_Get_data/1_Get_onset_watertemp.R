...
###############SAME DATA WRANGLING BUT FOR SITE 2
#limit logger data to sampling years and remove 30 min data in 2009 so consistent with other years
dat1 <- dat %>%
  mutate(date = date(datetime)) %>%
  filter(year %in% 2009:2016) %>%
  ...
  filter(min!=30) %>%
  select(-min) %>%
  select(datetime, date, year, dayofyr, time, SOTF_tempC)

# plot data to check
plot(SOTF_tempC ~ datetime, data = dat1)
...
dat4 <- sotf_watertemp_daily_summary %>%
  rename(SOTF.tempC_mean = SOTF_tempC, SOTF.tempC_median = SOTF_tempC1, SOTF.tempC_min = SOTF_tempC2, SOTF.tempC_max = SOTF_tempC3,SOTF.tempC_sd = SOTF_tempC4)
...
write_csv(sampling_dates_lag, "./00_Data_files/Covariate_analysis_data/sampling_dates_lag_SOTF.csv")
...
dat3_ma1 <- dat3_ma %>%
  mutate(ma_3 = rollmean(SOTF_tempC, k = 72, fill = NA, align = "right")) %>% #k = hours; 72 hours = 3 days
  mutate(ma_5 = rollmean(SOTF_tempC, k = 120, fill = NA, align = "right")) %>%
  mutate(ma_7 = rollmean(SOTF_tempC, k = 168, fill = NA, align = "right")) %>%
  mutate(ma_10 = rollmean(SOTF_tempC, k = 240, fill = NA, align = "right")) %>%
  mutate(ma_14 = rollmean(SOTF_tempC, k = 336, fill = NA, align = "right"))
